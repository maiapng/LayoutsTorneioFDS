LoadEverything().then(() => {
  let startingAnimation = gsap
    ...
    .from([".p1.container"], { duration: 0.5, y: "1200px", ease: "out" }, 0)
    .from([".p2.container"], { duration: 0.5, y: "1200px", ease: "out" }, 0)
    // .from(
    //   [".tournament"],
  ...
  Update = async (event) => {
    ...
    if (!isTeams) {
      ...
      for (const [t, team] of teams.entries()) {
        ...
        let htmlaaa = `<div class="players">`
        for (const [p, player] of Object.values(team.player).entries()) {
          let hearts = 
          player.real_name == "2" ? `<div class="heart full"></div><div class="heart full"></div>`
            : player.real_name == "1" ? `<div class="heart full"></div><div class="heart empty"></div>`
        ...
        htmlaaa += `</div>`
        SetInnerHtml($(`.p${t + 1}.team`), htmlaaa);
